        ...
        if (!empty($param)) {
            ...
            foreach($res['list'] as  $k=>$v){
                ...
                if(!empty($param['ck_type']) && !empty($param['val_type'])){
                    $update['type_id'] = $param['val_type'];
                    $type_list = model('Type')->getCache();
                    $id1 = intval($type_list[$param['val_type']]['type_pid']);
                    $update['type_id_1'] = $id1;
                    $des .= '&nbsp;'.lang('type').'：'.$param['val_type'].'；';
                }

                // 新增：批量修改标签
                if(!empty($param['ck_tag']) && !empty($param['val_tag'])){
                    $update['vod_tag'] = $param['val_tag'];
                    $des .= '&nbsp;'.lang('tag').'：'.$param['val_tag'].'；';
                }

                // 新增：批量修改播放源
                if(!empty($param['ck_play']) && !empty($param['val_play'])){
                    $update['vod_play_from'] = $param['val_play'];
                    // 同时清空播放服务器、备注和URL，保持一致性
                    $update['vod_play_server'] = '';
                    $update['vod_play_note'] = '';
                    $update['vod_play_url'] = '';
